...
def open_url_in_chrome(url):
    ...
    for index, level_range in enumerate(li_bestiary_list):
        ...
        # aqui precisa checar se é o ultimo link para poder pegar todas as tabelas
        if last_page_check:
            print('entrou na ultima')
            final_bestiary_page_all_monsters_tables = driver.find_elements(by=By.TAG_NAME, value='table')
            print(final_bestiary_page_all_monsters_tables, len(final_bestiary_page_all_monsters_tables))
            for last_monster_table_index, table in enumerate(final_bestiary_page_all_monsters_tables):
                if last_monster_table_index == len(final_bestiary_page_all_monsters_tables):
                    break
                print('entrou na last: ', last_monster_table_index)
                complete_monster_table = table
                complete_monster_table_headers = table.find_elements(by=By.TAG_NAME, value='th')
